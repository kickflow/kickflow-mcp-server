...
export const ListAuditLogsResponseItem = zod.object({
  ...
  "id": zod.uuid().describe('UUID'),
  "email": zod.email().max(listAuditLogsResponseUserOneEmailMax).describe('メールアドレス'),
  "code": zod.string().max(listAuditLogsResponseUserOneCodeMax).describe('コード'),
  "firstName": zod.string().max(listAuditLogsResponseUserOneFirstNameMax).describe('名'),
  "lastName": zod.string().max(listAuditLogsResponseUserOneLastNameMax).describe('姓'),
  "fullName": zod.string().max(listAuditLogsResponseUserOneFullNameMax).describe('フルネーム'),
  "employeeId": zod.string().max(listAuditLogsResponseUserOneEmployeeIdMax).nullish().describe('社員番号'),
  "image": zod.object({
